...
Scenario: Change activity date
  ...
  Then the activity with name "Build bench" has start date "uge 11 year 2022" and end date "week 16 year 2022"

Scenario: Add employee with 20 activites to a new activity
  Given an activity with name "Build bench"
  When an employee with UID "XX02" who has 20 ongoin activites tries to join the activity
...
Scenario: Edit previously registered work hours
  ...
  When an employee changes the registered time from 10 hours to 11 hours
